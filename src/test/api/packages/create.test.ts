...
import { POST } from "@/pages/api/packages/create";
...
describe("POST /api/packages/create", () => {
    it("returns 401 when not authenticated", async () => {
        ...
        expect(mockStorage.addProjectPackage).not.toHaveBeenCalled();
    });

    it("creates a package and returns 201", async () => {
        const body = { name: "numpy", projectId: 1 };
        const created = { id: 1, ...body, clerkUserId: "user_123" };
        mockStorage.addProjectPackage.mockResolvedValue(created);
        ...
        const res = await POST(ctx);
        await expectJson(res, 201, { id: 1, name: "numpy" });
        expect(mockStorage.addProjectPackage).toHaveBeenCalledWith({
        name: "numpy",
        projectId: 1,
    ...
    it("returns 400 when database rejects the input", async () => {
        mockStorage.addProjectPackage.mockRejectedValue(new Error("DB constraint"));
