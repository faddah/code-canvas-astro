...
import { GET } from "@/pages/api/starter-files/index";

beforeEach(() => {
    vi.clearAllMocks();
});

describe("GET /api/starter-files", () => {
    it("returns existing starter files", async () => {
            const files = [
                { id: 1, name: "main.py", content: "print('hi')" },
            ];
            mockStorage.getStarterFiles.mockResolvedValue(files);
    ...
    it("returns 200 with empty array when getStarterFiles returns empty after seeding", async () => {
            mockStorage.getStarterFiles.mockResolvedValue([]);
            mockStorage.createFile.mockResolvedValue({});
